...
export const useFooterSettings = () => {
  ...
  const getSetting = (settingName: string) => {
    ...
    console.log(`Footer getSetting for ${settingName}:`, value);
    return value;
  };

  const updateSetting = async (settingName: string, newValue: any) => {
    try {
      console.log(`Updating footer setting ${settingName} with:`, newValue);
      ...
      console.log(`Footer setting ${settingName} updated successfully in local state`);
      return { success: true };
    } catch (err) {
      console.error('Error updating footer setting:', err);
